...
class PurchasePlanDetail extends BaseModel
{
	...
	{
		return $this->hasOne('Bluebanner\Core\Model\PurchasePlanOrderLog', 'plan_detail_id');
	}
  ...
  public function getByPlanAndConds($conds) {
    $result = $this->with('item.quotations.vendor', 'item.primary.vendor')->has('orderdetail', 0)->conds($conds)->get();
