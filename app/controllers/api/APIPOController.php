...
class APIPOController extends \BaseController {
	...
	public function store() {
		...
		unset($order['details'], $order['vendor']);
		$order['agent'] = Sentry::getUser()->id;

		$object = PurchaseOrder::create($order);

		foreach ($details as $detail) {
			unset($detail['item']);
			$pod = PurchaseOrderDetail::create(array(
				'order_id' => $object->id,
				'item_id' => $detail['item_id'],
				'size' => $detail['size'],
				'qty' => $detail['qty'],
				'qty_confirmed' => $detail['qty_confirmed'],
				'qty_free' => $detail['qty_free'],
				'um' => $detail['um'],
				'tax_unit_price' => $detail['tax_unit_price'],
				'usd_unit_price' => $detail['usd_unit_price'],
				'unit_price' => $detail['unit_price'],
				'discount' => $detail['discount'],
				'total' => $detail['total'],
				'specification' => $detail['specification'],
			));
			PurchasePlanOrderLog::create(array('plan_detail_id' => $detail['id'], 'order_detail_id' => $pod->id));
		}
